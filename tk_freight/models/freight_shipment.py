...
class FreightShipment(models.Model):
    ...
    # Notify Party
    first_notify_id = fields.Many2one('res.partner', string="1st Notify", domain="[('notify','=',True)]")
    second_notify_id = fields.Many2one('res.partner', string="2nd Notify", domain="[('notify','=',True)]")
    ...
    def _compute_total_amount(self):
        for order in self:
            ...
            order.actual_margin = invoice_amount - invoice_residual - (-(bill_amount - bills_residual))
            order.margin = invoice_amount - (-bill_amount)
            order.accountancy_currency = "Amounts in " + str(order.company_id.currency_id.symbol)
    ...
    def _onchange_freight_packages(self):
        for rec in self:
            ...
            if rec.freight_packages:
                for data in rec.freight_packages:
                    text = text + str(data.qty) + " X " + str(data.package.name) + " - " + str(data.name) + "\n"
    ...
    def create(self, values):
        if values.get('name', ('New')) == ('New'):
            if values.get('transport') == 'air':
                air_prefix = self.env['ir.config_parameter'].sudo().get_param('tk_freight.air_seq')
                air_pre = str(air_prefix) if air_prefix else "AIR"
                values['name'] = air_pre + self.env['ir.sequence'].next_by_code('operation.master') or _('New')
            elif values.get('transport') == 'ocean':
                ocean_prefix = self.env['ir.config_parameter'].sudo().get_param('tk_freight.ocean_seq')
                ocean_pre = str(ocean_prefix) if ocean_prefix else "OCEAN"
                values['name'] = ocean_pre + self.env['ir.sequence'].next_by_code('operation.house') or _('New')
            elif values.get('transport') == 'land':
                land_prefix = self.env['ir.config_parameter'].sudo().get_param('tk_freight.land_seq')
                land_pre = str(land_prefix) if land_prefix else "LAND"
                values['name'] = land_pre + self.env['ir.sequence'].next_by_code('operation.direct') or _('New')
                            ...
                            if not data.invoiced:
                                if data.shipper_id.id == shipper:
                                    if data.currency_id.id == c:
                                        ...
                                        shipper_invoice_line.append((0, 0, shipper_invoice_record))
                            ...
                            if not data.invoiced:
                                if data.consignee_id.id == consignee:
                                    if data.currency_id.id == c:
                                        ...
                                        consignee_invoice_line.append((0, 0, consignee_invoice_record))
