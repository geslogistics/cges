...
class FreightService(models.Model):
    ...
    route_id = fields.Many2one('freight.route', 'Route')
    # Services
    service_type = fields.Selection([('shipper', 'Shipper'), ('consignee', 'Consignee'), ('vendor', 'Vendor')],
    ...
    status = fields.Selection([('bill', 'Bill Created'), ('invoice', 'Invoice Created'), ('pending', 'Pending')],
                              default="pending", readonly=True)
    # Invoice
    shipper_id = fields.Many2one('res.partner', string="Shipper",
                                 domain="[('shipper','=',True)]")
    consignee_id = fields.Many2one('res.partner', string="Consignee", domain="[('consignee','=',True)]")
    ...
    vendor = fields.Selection([('single', 'Single Vendor'), ('multiple', 'Multiple Vendor')], string='Vendor ',
                              store=True)
    vendor_id = fields.Many2one('res.partner', domain="['|',('notify','=',True),('vendor','=',True)]")
...
class TrackingTemplate(models.Model):
    ...
    name = fields.Char(string="Title")
    template_ids = fields.One2many('tracking.template.line', 'template_id', string="Template")
