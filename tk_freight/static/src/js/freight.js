odoo.define('tk_freight.FreightDashboard', function (require) {
  ...
  const ActionMenu = AbstractAction.extend({
    ...
    events: {
      ...
      'click .air-shipment': 'view_air_shipment',
      'click .ocean-shipment': 'view_ocean_shipment',
      'click .land-shipment': 'view_land_shipment',
    },
      ...
      $.when(this._super())
        .then(function (ev) {
          rpc.query({
            ...
            $('#air').empty().append(result['air']);
            $('#ocean').empty().append(result['ocean']);
            $('#land').empty().append(result['land']);
    ...
    view_air_shipment: function (ev) {
      ev.preventDefault();
      return this.do_action({
        name: _t('Air Shipment'),
        type: 'ir.actions.act_window',
        res_model: 'freight.shipment',
        domain: [['transport', '=', 'air']],
        context: { 'default_transport': 'air' },
        views: [[false, 'kanban'], [false, 'list'], [false, 'form']],
        target: 'current'
      });
    },

    view_ocean_shipment: function (ev) {
      ev.preventDefault();
      return this.do_action({
        name: _t('Ocean Shipment'),
        type: 'ir.actions.act_window',
        res_model: 'freight.shipment',
        domain: [['transport', '=', 'ocean']],
        context: { 'default_transport': 'ocean' },
        views: [[false, 'kanban'], [false, 'list'], [false, 'form']],
        target: 'current'
      });
    },
    view_land_shipment: function (ev) {
      ev.preventDefault();
      return this.do_action({
        name: _t('Land Shipment'),
        type: 'ir.actions.act_window',
        res_model: 'freight.shipment',
        domain: [['transport', '=', 'land']],
        context: { 'default_transport': 'land' },
        views: [[false, 'kanban'], [false, 'list'], [false, 'form']],
        target: 'current'
      });
    },
